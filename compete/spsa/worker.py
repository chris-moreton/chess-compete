...
def get_pending_iteration():
    ...
    - status='ref_pending': Reference phase (base vs Stockfish)

    Returns iteration data dict with 'phase' key or None if no work available.
    """
...
def ensure_spsa_engines_built(iteration: dict, config: dict, force_rebuild: bool = False) -> tuple[str, str]:
    """
    Ensure SPSA engine binaries (plus/minus) exist, building from database parameters if needed.

    Used during SPSA phase (plus vs minus games).

    Args:
        iteration: Iteration data from database (must be SPSA phase)
...
def ensure_base_engine_ready(iteration: dict, config: dict) -> str:
    ...
    SPSA games complete. Workers just use the path from the iteration record.
    ...
    """
    # The master builds the base engine and stores the path in the iteration
    # We just need to verify it exists locally
    ...
    if not base_path.exists():
        # Base engine should have been built by master - if not present, we need to build it
        print(f"\n  Base engine not found, building from updated parameters...")
        src_path = get_rusty_rival_path(config)
...
def run_spsa_worker(concurrency: int = 1, batch_size: int = 10, poll_interval: int = 10):
    """
    Run the SPSA worker loop with two-phase support.

    Two-phase iteration:
    - Phase 1 (SPSA): Build plus/minus engines, play plus vs minus games
    - Phase 2 (Ref): Use base engine (built by master), play base vs Stockfish
    ...
    print(f"\n{'='*60}")
    print("SPSA WORKER MODE (Two-Phase)")
    print(f"{'='*60}")
