...
# Chess-Compete Ubuntu Native SPSA Worker Setup
# Usage: curl -sS https://raw.githubusercontent.com/chris-moreton/chess-compete/main/scripts/setup-ubuntu-worker.sh | bash
...
echo ""
echo "============================================"
echo "  Chess-Compete Ubuntu Worker Setup ($ARCH)"
echo "============================================"
echo ""
...
if [ "$ARCH" = "x86_64" ]; then
  ...
  TMPDIR_SF=$(mktemp -d)
  echo "  Downloading pre-built Stockfish for x86_64..."
  curl -sL "$SF_URL" -o "$TMPDIR_SF/stockfish.tar"
  tar -xf "$TMPDIR_SF/stockfish.tar" -C "$TMPDIR_SF"
  SF_BIN=$(find "$TMPDIR_SF" -name "stockfish" -type f -executable | head -1)
  if [ -z "$SF_BIN" ]; then
  ...
  echo "  Building Stockfish from source for aarch64 (this may take a few minutes)..."
  TMPDIR_SF=$(mktemp -d)
...
default_name="ubuntu-$(generate_name)"
