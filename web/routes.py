...
def register_routes(app):
    ...
    def spsa_generate_report():
        ...
        for p in sorted(params, key=lambda x: x.name):
            ...
            first_val = iterations[0].base_parameters.get(p.name, p.value) if iterations[0].base_parameters else p.value
            total_change = current_val - first_val
            pct_change = (total_change / abs(first_val) * 100) if first_val != 0 else 0
            param_range = p.max_value - p.min_value
            dist_to_min_pct = (current_val - p.min_value) / param_range * 100 if param_range > 0 else 50
            dist_to_max_pct = (p.max_value - current_val) / param_range * 100 if param_range > 0 else 50
            param_lines.append(
                f"  {active} {p.name}: current={current_val:.4f}, start={first_val:.4f}, "
                f"change={total_change:+.4f} ({pct_change:+.1f}%), "
                f"bounds=[{p.min_value}, {p.max_value}], step={p.step}, group={p.group}, "
                f"dist_to_bounds: min={dist_to_min_pct:.1f}%, max={dist_to_max_pct:.1f}%"
            )
            if dist_to_min_pct < 5 or dist_to_max_pct < 5:
                bound = "MIN" if dist_to_min_pct < dist_to_max_pct else "MAX"
                params_at_bounds.append(f"{p.name} -> {dist_to_min_pct:.1f}% from min, {dist_to_max_pct:.1f}% from max (near {bound})")

        # Ref Elo statistics
        ref_elos = [float(it.ref_elo_estimate) for it in iterations if it.ref_elo_estimate is not None]
        ref_games_per_iter = latest.ref_target_games or 0

        ref_stats = ""
        if ref_elos:
            mean_elo = sum(ref_elos) / len(ref_elos)
            variance = sum((e - mean_elo) ** 2 for e in ref_elos) / len(ref_elos)
            std_dev = variance ** 0.5
            elo_range = max(ref_elos) - min(ref_elos)
            ref_stats = (
                f"Ref Elo stats ({len(ref_elos)} measurements, {ref_games_per_iter} games each):\n"
                f"  Mean: {mean_elo:.1f}, Std Dev: {std_dev:.1f}, Range: {min(ref_elos):.0f}-{max(ref_elos):.0f} (spread: {elo_range:.0f})\n"
                f"  Per-measurement noise: ~{std_dev:.0f} Elo (this is the uncertainty on each individual ref Elo reading)\n"
            )
            # Trend: compare halves
            if len(ref_elos) >= 6:
                third = len(ref_elos) // 3
                first_third = ref_elos[:third]
                last_third = ref_elos[-third:]
                first_avg = sum(first_third) / len(first_third)
                last_avg = sum(last_third) / len(last_third)
                ref_stats += (
                    f"  First third avg ({len(first_third)} iters): {first_avg:.1f}\n"
                    f"  Last third avg ({len(last_third)} iters): {last_avg:.1f}\n"
                    f"  Difference: {last_avg - first_avg:+.1f} Elo "
                    f"(~{abs(last_avg - first_avg) / std_dev:.1f} std devs — "
                    f"{'likely significant' if abs(last_avg - first_avg) > std_dev else 'within noise'})\n"
                )

        # Iteration detail (last 20)
        ...
        for it in recent:
            ...
            iter_lines.append(
                f"  Iter {it.iteration_number}: elo_diff={elo_diff:+.1f}, ref_elo={ref_str}"
            )
        ...
        if len(iterations) >= 4:
            ...
            convergence_info = (
                f"Avg param update magnitude: early={early_update:.4f}%/iter, "
                f"recent={recent_update:.4f}%/iter"
            )

        # SPSA coefficient values at current iteration
        eff_k = max(1, latest.iteration_number - run.effective_iteration_offset)
        current_a_k = run.spsa_a / ((eff_k + run.spsa_big_a) ** run.spsa_alpha)
        current_c_k = run.spsa_c / (eff_k ** run.spsa_gamma)

        bounds_section = ""
        if params_at_bounds:
            bounds_section = "PARAMETERS NEAR BOUNDS (<5% of range):\n" + "\n".join(f"  {p}" for p in params_at_bounds)
        else:
            bounds_section = "No parameters are currently near their bounds."

        system_prompt = """You are an expert at SPSA parameter tuning for chess engines. You understand:

- SPSA uses random perturbations to estimate gradients. Each iteration's elo_diff (plus vs minus engine) is inherently noisy.
- Ref Elo is measured by playing a small number of games against a reference engine (e.g. Stockfish). With only ~100-150 games, each measurement has error bars of ±30-50 Elo. Do NOT over-interpret individual data points or short-term trends in ref Elo.
- The SPSA learning rate decays as a_k = a / (k + A)^alpha. The parameter A controls how many iterations the algorithm explores broadly before settling down. If iteration k is still small relative to A, the algorithm is in its exploratory phase — this is by design, not a problem.
- The perturbation size decays as c_k = c / k^gamma. Smaller c means smaller perturbations and more precise (but noisier) gradient estimates.
- "Convergence" at low iteration counts is often just noise. SPSA typically needs 100+ iterations to show clear trends, depending on the number of parameters and games per iteration.
- Move ordering parameters affect search efficiency, not evaluation. Gains from tuning them tend to be smaller (5-20 Elo) than from evaluation or search-pruning params.
- When the ref Elo spread (max - min) is large relative to any observed trend, the trend may not be statistically significant.

Your job is to give a sober, statistically-aware analysis. Don't jump to conclusions from noisy data. Be honest when there isn't enough data to tell. Quantify uncertainty where possible."""

        user_prompt = f"""Analyze this SPSA tuning run and provide an actionable report.
...
Games per SPSA iteration: {run.games_per_iteration}
Ref games per iteration: {ref_games_per_iter}
...
Current effective iteration: {eff_k}
Current a_k (learning rate): {current_a_k:.6f}
Current c_k (perturbation size): {current_c_k:.4f}

## Parameters (* = actively tuned)
...
## Please address:
1. **Has the run plateaued?** Consider the noise level vs any apparent trend. Is there statistically meaningful evidence of improvement (or lack thereof)? Don't over-interpret noisy ref Elo measurements.
2. **Is progress being made?** Are parameter values moving in consistent directions, or oscillating? Is the overall ref Elo trend distinguishable from noise?
3. **Bound issues** — Any params near bounds that might need widening?
4. **Hyperparameter advice** — Given the current iteration count relative to A, is the learning rate schedule appropriate? Should c, A, or other settings change?
5. **Should we continue or try a different parameter group?** Consider whether this parameter group (move ordering, evaluation, search pruning, etc.) is likely to yield meaningful Elo gains.
6. **Anything else** — Outliers, dead parameters, suspicious patterns.

Be specific, cite numbers, and be honest about uncertainty. 4-6 paragraphs."""
